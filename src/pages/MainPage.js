...
function MainPage() {
  ...
  const fetchNearbyTouristSpots = async (latitude, longitude) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5007/api/tourist-spots/nearby?latitude=${latitude}&longitude=${longitude}&limit=3`);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', paddingBottom: '20px' }}>
            {heritageData.map(heritage => (
              <div 
                ...
                onClick={() => {
                  // 모든 관광지를 DetailPage로 이동 (백엔드 연결 문제 해결 전까지)
